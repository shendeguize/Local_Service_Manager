...
def test_port_available_and_sticky_state(tmp_path, monkeypatch):
    monkeypatch.setattr(ports, "STATE_DIR", tmp_path)
...
def test_allocate_requested_and_auto_port(tmp_path, monkeypatch):
    monkeypatch.setattr(ports, "STATE_DIR", tmp_path)
...
def test_allocate_reports_exhaustion(tmp_path, monkeypatch):
    monkeypatch.setattr(ports, "STATE_DIR", tmp_path)
